...
int objRead::loadObj_normalize_center(const char* filename)
{
	...
	for (unsigned int i = 0; i < vertexIndices.size(); i++) {
		...
		colors = temp_vertices[vertexIndex - 1];
		colors.x = 0;
		colors.y = 0;
		colors.z = 1;

		outvertex.push_back(temp);
		outcolors.push_back(colors);
...
int objRead::loadObj_normalize_center1(const char* filename)
{
	FILE* objFile;
	...
	while (1) {
		...
		if (strcmp(lineHeader, "v") == 0) {
			...
			int matches = fscanf(objFile, "%d %d %d\n", &vertexIndex[0],  &vertexIndex[1],&vertexIndex[2]);
			...
			std::cout << vertexIndex[0] << std::endl;
			vertexIndices.push_back(vertexIndex[0]);
			vertexIndices.push_back(vertexIndex[1]);
			vertexIndices.push_back(vertexIndex[2]);
...
int objRead::loadObj_normalize_center2(const char* filename)
{
	FILE* objFile;
...
int objRead::loadObj_normalize_center3(const char* filename)
{
	FILE* objFile;
